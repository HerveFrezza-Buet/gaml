...
namespace gaml {
  namespace datasets {
     ...
     */     
    template<unsigned int INPUT_DIM, typename PARSER>
    ...
    public:
      ...
      static constexpr unsigned int input_dim = INPUT_DIM;
	...
	return dataset.end();
      ...
      static const input_type& input_of_data(const data_type& data) {
	return data.first;
      }

      static const output_type& output_of_data(const data_type& data) {
	return data.second;
      }      
    };
     ...
     */  
    template<unsigned int INPUT_DIM, typename PARSER>
    DownloadedDataset<INPUT_DIM, PARSER> make_downloaded(const std::string& url,
							 const PARSER& parser) {
      return DownloadedDataset<INPUT_DIM, PARSER>(url, parser);
    }
    ...
    inline auto make_iris() {
      ...
      auto parser = make_csv_parser<input_type, output_type>(',', 0, 5, parse_field);
      return make_downloaded<4>(url, parser);
    }
    ...
    inline auto make_diabetes() {
      ...
      auto parser = make_csv_parser<input_type, output_type>(' ', 1, 11, parse_field);
      return make_downloaded<10>(url, parser);
    }
    ...
    inline auto make_wine() {
      ...
      auto parser = make_csv_parser<input_type, output_type>(',', 0, 14, parse_field); 
      return make_downloaded<13>(url, parser);
     ...
     */
    inline auto make_boston_housing() {
