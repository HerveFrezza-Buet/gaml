...
namespace customer {
  ...
  using Gender = bool;                             // true = female, false = male.
  using Age = int;
  using Date = std::array<int, 3>;                 // (year, month, day)
  using Article = std::pair<std::string, double>;  // (reference name, price)
  using ShoppingBasket = std::vector<Article>;     // A std::list could also have been used.
  using Purchase = std::tuple<Date, ShoppingBasket>;
  using Ratings = std::map<std::string, int>;
  using Data = std::tuple<Gender, Age, std::vector<Purchase>, Ratings>;
