...
public:
  ...
  Predictor operator()(const DataIterator& begin, const DataIterator& end,
    ...
    auto it_true = trues.begin();
    std::advance(it_true,pos);
    ...
    auto it_false = falses.begin();
    std::advance(it_false,pos);
...
class SillyRandomizer {
  ...
  gaml::Shuffle<DataIterator> operator()(const DataIterator& begin, const DataIterator& end) const {
