...
int main(int argc, char* argv[]) {
  ...
  for(auto& data : basis) {
    double x = gaml::random::uniform(0,1);
    double y = RATIO*x + gaml::random::uniform(-NOISE,NOISE);
    data = {x,y};
  }
  ...
  auto leave_one_out_evaluator = gaml::risk::bootstrap::leave_one_out(gaml::loss::Quadratic<double>(), NB_BOOTSTRAPPED_SETS, verbosity);
  auto r632_evaluator          = gaml::risk::bootstrap::r632         (gaml::loss::Quadratic<double>(), NB_BOOTSTRAPPED_SETS, verbosity);
  auto r632plus_evaluator      = gaml::risk::bootstrap::r632plus     (gaml::loss::Quadratic<double>(), NB_BOOTSTRAPPED_SETS, verbosity);
