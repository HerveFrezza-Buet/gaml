...
namespace gaml {
  ...
  struct TupleComponentParser<Tuple, i, 0> {
    typedef Tuple tuple_type;
    TupleComponentParser(std::istream& is) {
    }
  ...
  template<typename Tuple, int i>
  class TupleComponentParser<Tuple, i, 1> {
    typedef Tuple tuple_type;
    typedef typename std::tuple_element<i, tuple_type>::type element_type;
    JSONParser<element_type> headParser_;
  public:
  ...
  template<typename ... Args> class JSONParser<std::tuple<Args...>> : public ParserBase {
    ...
    TupleComponentParser<tuple_type, 0, std::tuple_size<tuple_type>::value> tupleParser_;
  public:
