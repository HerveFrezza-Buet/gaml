...
namespace gaml {
  ...
  class Bootstrap : public Tabular<Iterator> {
  ...
  public:
    ...
    Bootstrap(void) : Tabular<Iterator>() {}
    Bootstrap(const Bootstrap<Iterator>& cp) : Tabular<Iterator>(cp) {}
    Bootstrap(const Iterator& begin_iter, 
	      const Iterator& end_iter,
	      unsigned int size) : Tabular<Iterator>(begin_iter) {
     ...
     * For internal use
     */
    const typename Tabular<Iterator>::indices_type get_indices(void) const {return this->indices;}
	...
	double operator()(const Learner& learner,const DataIterator& begin, const DataIterator& end,
	  ...
	  if(verbose) {
	    for(b = 0; b < nb_sets; ++b) {
	      ...
	      for(auto elem : sets[b].get_indices())
