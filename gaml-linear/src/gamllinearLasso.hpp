...
namespace gaml {
  namespace linear {
	 ...
	 * TODO : solving the wslope = (Phi^T Phi)^-1 s can be speed up by updating the inverse. However, in this algorithm, we have to consider both insertion and suppression
	 *        of columns
	 *
	 */
	...
	predictor_type operator() (const DataIterator &begin, const DataIterator &end, const InputOf & input_of, const OutputOf & output_of) const {
	  ...
	  if(verbose) std::cout << "Activating basis " << selected_dim << std::endl;
	  ...
	  while(!stop) {
	    ...
	    // 
	    double dlambdaj_active = std::numeric_limits<double>::lowest();
	    unsigned int selected_dim_active = 0;
	    ...
	    for(auto act: active_dim) {
	      if(act != last_dim) {
		double delta_lambda = w[act]/gsl_vector_get(wslope, current_dim);
		if(delta_lambda < 0 && delta_lambda > dlambdaj_active) {
		  dlambdaj_active = delta_lambda;
		  selected_dim_active = act;
	    ...
	    if(dlambdaj_inactive_neg >= 0 ||
	      ...
	      ostr << "gaml::linear::lasso::Learner :  Something strange is happening, one of the delta_lambdaj >= 0" 
		   << ", inactive neg : " << dlambdaj_inactive_neg 
		   << ", inactive pos : " << dlambdaj_inactive_pos
		   << ", active : "       << dlambdaj_active;
	      throw std::runtime_error(ostr.str());
	    }

	    if(!has_found_dim_candidate) {
	      if(verbose) std::cerr << "gaml::linear::lasso::Learner :  cannot find a candidate dimension, I stop learning" << std::endl;
	    ...
	    double delta_lambda;
	    bool activate_dimension = false;
	    double activation_sign = 0;
	    if((dlambdaj_active > dlambdaj_inactive_pos) && (dlambdaj_active > dlambdaj_inactive_neg)) {
	      // dlambda_active > (dlambdaj_inactive_pos , dlambdaj_inactive_neg)
	      // We inactivate selected_dim_active
	      delta_lambda = dlambdaj_active;
	      selected_dim = selected_dim_active;
		
	      activate_dimension = false;

	      if(verbose) std::cerr << "Inactivating basis " << selected_dim << std::endl;
	    }
	    else {
	      // At least we know    dlambdaj_active <= max(dlambdaj_inactive_neg, dlambdaj_inactive_pos)
	    ...
	    if(verbose) std::cout << "Delta lambda :" << delta_lambda << std::endl;

	    // lambda_j+1 = lambda_j + delta_lambda
	    lambda_cur = lambda_prev + delta_lambda;
	    if(verbose) std::cout << "lambda : "<< lambda_cur << std::endl;
	    ...
	    // Activate or deactivate the variables
	    if(activate_dimension) {
	      active_dim.insert(selected_dim);
	      inactive_dim.erase(selected_dim);
	      ...
	      active_dim.erase(selected_dim);
	      inactive_dim.insert(selected_dim);
	      
	      w.erase(selected_dim);
	      s.erase(selected_dim);
	    }
	  ...
	  auto predictor = build_predictor(mean_features, sigma_features, mean_y,
					   w, active_dim);
