  ...
  namespace xtree {
	...
	if(should_split(begin,end,input_of,output_of,nmin,out_attr)) {
	  ...
	  for(auto attr_it = attr_begin; attr_it != attr_end; ++attr_it) {
	    ...
	    ThresholdTest<X> test(gaml::random::uniform(*(bounds.first),*(bounds.second)),a);
	  ...
	  auto filter_function = [&input_of,&best_test](const typename DataIterator::value_type& data) {return best_test(input_of(data));};
